...
**NOT for:**
- Diagnosing a failure happening right now — that is incident response; this skill is what makes that fast next time
...
- Making a repeated effect harmless — see `idempotency-and-exactly-once`, which owns the "unknown" column this skill keeps producing
- Designing telemetry — this skill tells you *what* must be observable; instrumenting it is separate
...
1. **Make it safe to repeat** — an idempotency key, so retrying is harmless. Preferred. See `idempotency-and-exactly-once`.
...
The exit criterion is the same one that governs this whole pack: *if you cannot point at a test that fails when the handling is removed, the behavior is unspecified, whatever the document says.*
